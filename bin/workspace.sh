...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    --move-to)
      name=$(get_name "$2")
      swaymsg -q "move container to workspace $name"
      shift
      shift
      ;;
    --go-to)
      name=$(get_name "$2")
      swaymsg -q "workspace \"$name\""
      shift
      shift
      ;;
    --rename)
      ...
      swaymsg -q "rename workspace \"$current_workspace\" to \"$new_name\""
      shift
      shift
      ;;
    --setup)
      ...
      for name in $(jq -r 'to_entries | map(.key + if .value == "" then "" else ": " end + .value) | .[]' "$CONFIG_FILE"); do
        ...
        current_name=$(swaymsg -t 'get_workspaces' | jq -r ".[] | select(.num == $target_number).name")
        swaymsg -q "rename workspace \"$current_name\" to \"$name\""
      done
      IFS=$OFS
      shift
      ;;
